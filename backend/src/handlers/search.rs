...
use crate::{auth::AuthUser, storage::{find_docs_by_tag, list_all_tags, list_doc_names}};
...
pub async fn render_search_results(
    ...
    match query {
            ...
            match list_doc_names() {
                Ok(names) => {
                    ...
                    let html = if matches.is_empty() {
                        render_search_empty_html(keyword, &username)
                    } else {
                        // render_search_page(keyword, &matches)
                        render_search_result_html(keyword, &matches, &username)
                    };
        ...
        None => {
            Html(render_search_no_input_html(&username)).into_response()
        }
    }
}

pub async fn render_search_tags(
    Path(tag_name): Path<String>,
    AuthUser(username): AuthUser
) -> impl IntoResponse {
    match find_docs_by_tag(&tag_name) {
        Ok(mut matched_docs) => Html(render_search_tag_html(&tag_name, &mut matched_docs, &username)).into_response(),
        Err(_) => Html("<h1>문서 목록 불러오기 실패</h1>".to_string()).into_response(),
    }
}

pub async fn render_all_tags(
    AuthUser(username): AuthUser
) -> impl IntoResponse {
    let tags = list_all_tags().unwrap_or_default();
    Html(render_all_tags_html(&tags, &username))
}
